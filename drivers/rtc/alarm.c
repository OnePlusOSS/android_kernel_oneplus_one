...
#ifndef VENDOR_EDIT //mingqiang.guo@phone.bsp modify for power up at 32s before timing power up time  
#define ALARM_DELTA 120
#else //VENDOR_EDIT
#define ALARM_DELTA 32
#endif //VENDOR_EDIT
...
static int rtc_alarm_update(struct timespec *alarm)
{
    ...
    /* ADD BY yangrujin@oneplus.cn START. 2015/03/13*/
	/*
	 * Substract ALARM_DELTA from actual alarm time
	 * to powerup the device before actual alarm
	 * expiration.
	 */
	if ((rtc_alarm_time - ALARM_DELTA) > rtc_current_time){
		rtc_alarm_time -= ALARM_DELTA;
    }
    /* ADD BY yangrujin@oneplus.cn END */
